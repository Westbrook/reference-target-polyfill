...
| Combobox `aria-controls` and `aria-activedescendant` | Opt-in `comboboxTargets()` binds to a component-provided real listbox and active option in the input's own tree | Requires a public structural contract; it cannot reference private internal accessible nodes or supply the widget's interaction behavior. |
...
**Package and loading contract.** The local package is named `reference-target-fallback`; it has not been published to npm. It provides a synchronous core and separate adapter entry points. The application selects its adapter modules through static imports inside one setup module, loaded according to the required capability. The bootstrap awaits that module before importing the application, which can initialize at module top level without exporting `start()`. Regular adapters retain native Reference Target syntax; the combobox adapter uses the explicit public structural contract described below. The design discussion includes possible extensions; [README.md](./README.md) documents the implemented API.
...
| `adapters/combobox-targets` | Editable combobox relationships to a component's public listbox and active option | Synchronous public provider, actual same-tree elements, owned attribute bindings, and component-managed interaction. Remains needed with native Phase 1. |
...
**Combobox implementation decision — 31 August 2026.** The explainer's [active-descendant and combobox case](https://github.com/WICG/webcomponents/blob/gh-pages/proposals/reference-target-explainer.md#aria-activedescendant-and-comboboxes) is a useful boundary for this package. An external input must control the listbox while identifying a particular option as its active descendant. Forwarding both attributes through one host to one target cannot express those two destinations. The broader [Phase 2 section](https://github.com/WICG/webcomponents/blob/gh-pages/proposals/reference-target-explainer.md#phase-2-referring-to-specific-elements-within-a-shadow-root), [discussion #1111](https://github.com/WICG/webcomponents/issues/1111), and [November 2025 discussion](https://www.w3.org/2025/11/10-webcomponents-minutes.html#d323) consider additional target-selection mechanisms. They are not a basis for claiming Phase 2 support in this polyfill.

The implemented `comboboxTargets({ getTargets })` follows the structural approach above. Its synchronous provider receives a public host and returns `null` or `{ listbox, activeOption }`, with `activeOption` nullable. The host and actual listbox/options share the editable input's DOM tree. The listbox is a descendant of the host, and each active option is a DOM descendant of that listbox. Public light-DOM content can be presented through the component's shadow slot; no duplicate accessibility surface is manufactured.

The adapter accepts text/search inputs with `role="combobox"` and one host ID in `aria-controls`. It verifies the supplied roles, unique IDs, realm, connection, and tree relationships before binding `aria-controls` to the listbox. When the authored `aria-activedescendant` also references that host, it binds to the usable active option while expanded, or removes the effective attribute otherwise. It does not replace an absent or directly authored option reference, nor implement explicit ARIA element-reference setters. It tracks its own applied attributes, respects author replacements, and restores only values it still owns when released.

This produces ordinary native ARIA ID references to real nodes. It does not forward an active descendant transitively through the listbox, replace reflection getters, or mirror an option into hidden DOM. The [ARIA active-descendant requirements](https://w3c.github.io/aria/#aria-activedescendant) and [combobox authoring pattern](https://www.w3.org/WAI/ARIA/apg/patterns/combobox/) still apply. The component owns naming, rendering, filtering, expanded/selected state, keyboard and pointer interaction, focus, and scrolling. Its model updates call `refresh()` synchronously so the relationship follows the active option during keyboard handling. DOM assertions and visibility guards do not establish screen-reader interoperability.

Native Phase 1 does not remove this provider contract. Applications selecting this adapter await their selected setup and then import `app.js` regardless of the Phase 1 surface check. The adapter explicitly allows native cooperation; the core preserves native roots and `attachShadow()`, and other selected adapters retain their native status. A component can retire the bridge by authoring direct relationships to these public nodes, or by adopting a future supported multi-target capability. A component exposing only private listbox/options still needs a structural change; this addition does not transparently polyfill that encapsulated case.
...
The implementation covers registration/resolution, selected label interactions, dialog/popover actions, opt-in plain-text names/descriptions, submit/reset actions referencing an inner native form, and cooperative combobox relationships to real public nodes. The [current README](./README.md) documents all seven adapters and their limits. Arbitrary private cross-shadow active-descendant, ownership, and full native form association remain outside its scope.
